...
with open('fixtures.yml') as yaml_file:
    fixtures = yaml.safe_load(yaml_file)

params = [(eval(fixture["long"]), eval(fixture["short"]), [eval(range) for range in fixture["expected"]]) for fixture in fixtures.values()]


@pytest.mark.parametrize("large, short, expected", params)
